...
function formatAgeMs(value: number) {
...
export function EventInspector({ event, onClose }: { event: InferenceEvent | null; onClose: () => void }) {
  ...
  const rows = [
    ...
    ['RTT measurement age', formatAgeMs(event.rtt_age_ms)],
  ...
  return (
    <div className="inspector-backdrop" onMouseDown={onClose}>
      <aside className="inspector" onMouseDown={(e) => e.stopPropagation()}>
        ...
        <div className="provenance-box"><strong>Measurement integrity</strong><p>RTT is an application-level probe measurement and may be reused from the last successful probe; its source and age are shown above. RSSI, energy, pure network latency and total E2E latency are not populated by the current production telemetry contract. The dashboard leaves them unavailable rather than estimating them.</p></div>
      </aside>
    </div>
  )
}
