// Isolated Split2 integration test; production inference is unchanged.
#include <Arduino.h>
#include <unity.h>
#include <cmath>
#include <cstring>
...
constexpr const char* requestTopic = "gesture/esp32-split2-test/inference/request";
constexpr const char* responseTopic = "gesture/esp32-split2-test/inference/response";
char response[512];
...
void testSplit2EndToEnd() {
    static tflite::MicroMutableOpResolver<1> resolver;
    TEST_ASSERT_EQUAL(kTfLiteOk, resolver.AddFullyConnected());
    static tflite::MicroInterpreter interpreter(
        tflite::GetModel(split2_model_data::MODEL), resolver, arena, sizeof(arena));
    ...
    TEST_ASSERT_TRUE(network::connectMqtt("esp32-split2-e2e"));
    TEST_ASSERT_TRUE(network::subscribeMqtt(responseTopic));

    for (size_t v = 0; v < split2_parity_vectors::VECTOR_COUNT; ++v) {
        memcpy(interpreter.input(0)->data.f, split2_parity_vectors::NORMALIZED_INPUTS[v],
               10 * sizeof(float));
        TEST_ASSERT_EQUAL(kTfLiteOk, interpreter.Invoke());
        String payload = "{\"request_id\":\"split2-e2e-" + String(v) +
            "\",\"device_id\":\"esp32-split2-test\",\"timestamp_ms\":" + String(millis()) +
            ",\"split\":2,\"model_version\":\"gesture-model-v1.1.0\",\"embedding\":[";
        for (size_t i = 0; i < 48; ++i) {
            const float value = interpreter.output(0)->data.f[i];
            TEST_ASSERT_TRUE(std::isfinite(value));
            TEST_ASSERT_FLOAT_WITHIN(1e-5f, split2_parity_vectors::EXPECTED_OUTPUTS[v][i], value);
            char number[32];
        ...
        snprintf(expectedRequest, sizeof(expectedRequest), "\"request_id\":\"split2-e2e-%u\"", unsigned(v));
        received = false;
        ...
        TEST_ASSERT_TRUE_MESSAGE(received, "Split2 server response timeout");
        TEST_ASSERT_NOT_NULL(strstr(response, "\"split\":2"));
        TEST_ASSERT_NOT_NULL(strstr(response, "\"model_version\":\"gesture-cloud-tail-split2-v1.0.0\""));
        ...
        Serial.printf("SPLIT2_E2E_VECTOR=%u RESPONSE=%s\n", unsigned(v), response);
    }
    network::disconnectMqtt();
    Serial.println("PHASE7_SPLIT2_E2E_PASS");
}
}  // namespace
