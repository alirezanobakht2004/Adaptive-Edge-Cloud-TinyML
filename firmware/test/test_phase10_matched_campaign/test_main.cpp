// Isolated matched-state experiment, derived from the Phase9 test harness.
// One physical decision snapshot; separate live guards expose sequential drift.
#include <Arduino.h>
#include <unity.h>
#include <cmath>
#include <cstring>
...
void campaign(String command) {
    ...
    token = strtok(nullptr, " "); if (!token) return; int rotation = atoi(token);
    token = strtok(nullptr, " "); if (!token) return; int repeats = atoi(token);
    if (rotation < 0 || rotation > 3 || repeats < 1 || repeats > 16) return;
    ...
    const uint32_t sharedUs = micros() - totalStart;
    const uint32_t snapshotUs = micros();
    const int snapshotRssi = network::wifiRssiDbm();
    for (int repeat = 0; repeat < repeats; ++repeat) {
    for (int position = 0; position < 4; ++position) {
    const int action = (rotation + repeat + position) % 4;
    id = "m30-" + String(millis()) + "-" + String(window) + "-" + String(repeat) + "-" + String(action);
    uint32_t guardUs = 0, guardSent = 0;
    String guard = "{\"request_id\":\"" + id + "-guard\",\"probe\":true}";
    bool guardOk = exchange(guard, id + "-guard", guardUs, guardSent);
    const unsigned guardHeap = ESP.getFreeHeap(), guardPsram = ESP.getFreePsram();
    const int guardRssi = network::wifiRssiDbm();
    const unsigned guardCpu = getCpuFrequencyMhz();
    const bool guardConnected = network::isMqttConnected();
    const uint32_t ageUs = micros() - snapshotUs;
    ...
    uint32_t actionUs = micros() - started, totalUs = sharedUs + actionUs;
    Serial.printf("MATCH_META {\"window_id\":%u,\"repeat\":%d,\"position\":%d,\"action\":%d,\"shared_us\":%u,\"snapshot_us\":%u,\"age_us\":%u,", window, repeat, position, action, sharedUs, snapshotUs, ageUs);
    Serial.printf("\"snapshot_rssi\":%d,\"guard_heap\":%u,\"guard_psram\":%u,\"guard_rssi\":%d,\"guard_cpu\":%u,\"guard_connected\":%s,\"guard_rtt_us\":%s}\n", snapshotRssi, guardHeap, guardPsram, guardRssi, guardCpu, guardConnected ? "true" : "false", guardOk ? String(guardUs).c_str() : "null");
    ...
    Serial.printf("\"probe_us\":%s,\"connected\":%s,\"rssi_dbm\":%d,", probeOk ? String(probeUs).c_str() : "null", connected ? "true" : "false", snapshotRssi);
...
void setup() {
    ...
    Serial.setTimeout(1000); Serial.println("MATCHED_CAMPAIGN_READY");
}
void loop() {
    network::mqttLoop();
    if (Serial.available()) { String line = Serial.readStringUntil('\n'); line.trim(); if (line == "PING") Serial.println("MATCHED_CAMPAIGN_READY"); else if (line.startsWith("MATCH ")) campaign(line); }
    delay(1);
}
