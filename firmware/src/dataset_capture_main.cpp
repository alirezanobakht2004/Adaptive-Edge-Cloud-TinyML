...
constexpr const char* COLLECTION_DATASET_VERSION = "dataset-v2";
...
void setup() {
    ...
    delay(1500);
    Serial.println();
    Serial.println("=== Phase 12 / Continual-Learning Dataset Capture ===");
    Serial.printf("Firmware version: %s\n", FIRMWARE_VERSION);
    Serial.printf("Accelerometer calibration: %s\n", ACCEL_CALIBRATION_VERSION);
    Serial.printf("Orientation protocol: %s\n", ORIENTATION_VERSION);
    Serial.printf("Dataset target: %s\n", COLLECTION_DATASET_VERSION);

    if (!Wire.begin(SDA_PIN, SCL_PIN, I2C_FREQUENCY_HZ)) {
    ...
    if (whoAmI == 0x68) {
        Serial.println("Sensor identity: standard MPU6050");
    } else if (whoAmI == 0x74) {
        Serial.println(
        ...
        Serial.println("FATAL: unsupported sensor identity.");
        while (true) {
            delay(1000);
        }
    }
    ...
    if (!mpu6050Begin()) {
        Serial.println("FATAL: sensor configuration failed.");
        while (true) {
            delay(1000);
        }
    }
    ...
    GyroBias bias;
    if (!mpu6050CalibrateGyro(bias, GYRO_CALIBRATION_SAMPLES)) {
        Serial.println("FATAL: gyro calibration failed.");
        while (true) {
            delay(1000);
        }
    }
